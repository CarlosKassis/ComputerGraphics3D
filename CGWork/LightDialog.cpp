...
CLightDialog::CLightDialog(CWnd* pParent /*=NULL*/)
	...
	, m_dAmbientIntensity(0)
	, m_dDiffuseIntensity(0)
	, m_dSpecularIntensity(0)
	, m_nSpecularExponent(0)
{
    m_currentLightIdx = 0;
}
...
void CLightDialog::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_AMB_INTENSITY, m_dAmbientIntensity);
	DDV_MinMaxInt(pDX, m_dAmbientIntensity, 0, 1);
	DDX_Text(pDX, IDC_DIFF_INTENSITY, m_dDiffuseIntensity);
	DDV_MinMaxInt(pDX, m_dDiffuseIntensity, 0, 1);
	DDX_Text(pDX, IDC_SPEC_INTENSITY, m_dSpecularIntensity);
	DDV_MinMaxInt(pDX, m_dSpecularIntensity, 0, 10);
	DDX_Text(pDX, IDC_SPEC_EXPONENT, m_nSpecularExponent);
	DDV_MinMaxInt(pDX, m_nSpecularExponent, 2, 128);
}
...
void CLightDialog::SetDialogGlobals(double ambientIntensity, double diffuseIntensity, double specularIntensity, int specuralExponent)
{
	m_dAmbientIntensity = ambientIntensity;
	m_dDiffuseIntensity = diffuseIntensity;
	m_dSpecularIntensity = specularIntensity;
	m_nSpecularExponent = specuralExponent;
}
...
void CLightDialog::GetDialogGlobals(double& ambientIntensity, double& diffuseIntensity, double& specularIntensity, int& specuralExponent)
{
	ambientIntensity = m_dAmbientIntensity;
	diffuseIntensity = m_dDiffuseIntensity;
	specularIntensity = m_dSpecularIntensity;
	specuralExponent = m_nSpecularExponent;
}
